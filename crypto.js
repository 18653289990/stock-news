...
function renderCryptoCards(bySymbol) {
    ...
    container.innerHTML = MAIN_COINS.map((coin, i) => {
        ...
        const pctColor = pct === null ? 'text-gray-400' : isUp ? 'crypto-up' : 'crypto-down';
        const arrow = pct === null ? '' : isUp ? '▲ ' : '▼ ';

        return `<div class="flex-1 text-center px-3 py-2 ${flashCls} min-w-0">
            <div class="flex items-center justify-center gap-1 mb-1">
                <span class="text-sm font-bold text-gray-900">${coin.symbol}</span>
            </div>
            <div class="text-base font-bold tabular-nums text-gray-900 leading-tight">$${priceStr}</div>
            <div class="text-xs mt-0.5 ${pctColor}">${arrow}${pctHtml}</div>
        </div>`;
    }).join('');
}
